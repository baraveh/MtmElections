...
int* copyPointer(int* ptr);
void destroyPointer (int* ptr);
bool pointerIsEqual (int* pointerOne, int* pointerTwo);
bool pointerIsGreater (int* pointerOne, int* pointerTwo);
...
MtmElectionsResult mtmElectionsAddCitizen(MtmElections mtmElections, const char *citizenName,
    ...
    CityResult addResult =
            cityAddCitizen(toAddTo, citizenId, citizenName,
                    yearsOfEducation, citizenAge);
...
MtmElectionsResult MtmElectionsCitizenGetEducation(MtmElections mtmElections, int citizenId, int *yearsOfEducation) {
    ...
    City iterator = uniqueOrderedListGetLowest(mtmElections->cities);
    while (iterator != NULL) {
        CityResult getResult =
                getACitizenEducation(iterator, citizenId, yearsOfEducation);
        if (getResult == CITY_SUCCESS) {
            return MTM_ELECTIONS_SUCCESS;
        }
...
MtmElectionsResult mtmElectionsAddCandidate(MtmElections mtmElections, int candidateId, int cityId) {
    ...
    City cityToMakeIn;
    MtmElectionsResult getResult =
            getACity(mtmElections, cityId, &cityToMakeIn);
    if (getResult == MTM_ELECTIONS_CITY_DOES_NOT_EXIST) {
        return MTM_ELECTIONS_CITY_DOES_NOT_EXIST;
    }
...
MtmElectionsResult mtmElectionsWithdrawCandidate(MtmElections mtmElections, int candidateId, int cityId) {
    ...
    CityResult withdrawResult =
            cityWithdrawCandidate(cityToMakeIn, candidateId);
...
mtmElectionsSupportCandidate(MtmElections mtmElections, int citizenId, int candidateId, int priority) {
    ...
    int citizenCityId;
    int candidateCityId;
    MtmElectionsResult citizenGet =
            MtmElectionsCitizenGetCity(mtmElections, citizenId, &citizenCityId);
    MtmElectionsResult candidateGet =
            MtmElectionsCitizenGetCity(mtmElections, candidateId, &candidateCityId);
    ...
    if (addResult == CITY_NO_SUCH_CANDIDATE || addResult == CITY_CITIZEN_IS_NOT_A_CANDIDATE) {
        return MTM_ELECTIONS_CANDIDATE_DOES_NOT_EXIST;
    }
    ...
    if(addResult == CITY_NO_SUCH_CITIZEN){
        return MTM_ELECTIONS_CITIZEN_DOES_NOT_EXIST;
    }
...
MtmElectionsResult mtmElectionsCancelSupport(MtmElections mtmElections, int citizenId, int candidateId) {
    ...
    City candidateCity;
    MtmElectionsResult citizenGet =
            getACity(mtmElections, citizenId, &citizenCity);
    MtmElectionsResult candidateGet =
            getACity(mtmElections, candidateId, &candidateCity);
    if (citizenGet == MTM_ELECTIONS_CITY_DOES_NOT_EXIST) {
        return MTM_ELECTIONS_CITIZEN_DOES_NOT_EXIST;
    }
    if (candidateGet == MTM_ELECTIONS_CITY_DOES_NOT_EXIST) {
        return MTM_ELECTIONS_CANDIDATE_DOES_NOT_EXIST;
    }
    assert(citizenGet == MTM_ELECTIONS_SUCCESS);
    assert(candidateGet == MTM_ELECTIONS_SUCCESS);
...
MtmElectionsResult mtmElectionsChangeAddress(MtmElections mtmElections, int citizenId, int cityId) {
    ...
    City toMoveFrom;
    City toMoveTo;
    MtmElectionsResult idGetResult =
            MtmElectionsCitizenGetCity(mtmElections, citizenId, &cityIdToMoveFrom);
    MtmElectionsResult cityGetResult =
            getACity(mtmElections, cityId, &toMoveTo);
    ...
    MtmElectionsResult getResult =
            getACity(mtmElections, cityIdToMoveFrom, &toMoveFrom);
    assert(getResult == MTM_ELECTIONS_SUCCESS);

    int citizenAge;
    int citizenEducation;
    char *citizenName;
    CityResult getEducation =
            getACitizenEducation(toMoveFrom, citizenId, &citizenEducation);
    ...
    CityResult addResult = cityAddCitizen(toMoveTo, citizenId,
            citizenName, citizenEducation, citizenAge);
    if (addResult == CITY_MEMORY_ERROR) {
        return MTM_ELECTIONS_MEMORY_ERROR;
    }
    assert(addResult == CITY_SUCCESS);
...
int mtmElectionsRankByAge(MtmElections mtmElections, int citizen, void *pAge) {
    ...
    int citizenAge;
    MtmElectionsResult getResult =
            MtmElectionsCitizenGetAge(mtmElections, citizen, &citizenAge);
...
UniqueOrderedList mtmElectionsPerformElections(MtmElections mtmElections,
    ...
    while (iterator != NULL) {
        ...
        if (electionsResult == MTM_ELECTIONS_SUCCESS) {
            ...
            if(printMayor(mtmElections, cityId, mayorId, filename)
    ...
    UniqueOrderedList sortedWinners =
            uniqueOrderedListCreate((void *(*)(void *)) copyPointer,
                    (void (*)(void *)) destroyPointer,
                    (bool (*)(void *, void *)) pointerIsEqual,
                    (bool (*)(void *, void *)) pointerIsGreater);
    Candidate currentWinner = uniqueOrderedListGetLowest(winners);
    while(currentWinner!=NULL){
        int* toInsert = malloc(sizeof(*toInsert));
        candidateGetCandidateId(currentWinner, toInsert);
        uniqueOrderedListInsert(sortedWinners, toInsert);
        currentWinner = uniqueOrderedListGetNext(winners);
    }

    uniqueOrderedListDestroy(winners);
...
MtmElectionsResult mtmElectionsMayorOfCity(MtmElections mtmElections, int cityId, int *mayor, const char* filename) {
    ...
    return printMayor(mtmElections, cityId, *mayor, filename);
}
...
MtmElectionsResult printMayor (MtmElections mtmElections, int cityId, int mayorId, const char* filename){
    ...
    mtmPrintMayorDetails(mayorId, age, yearsOfEducation,
            citizenName, cityName, cityId, stream);
    fclose(stream);
